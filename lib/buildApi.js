...
function buildApi(endpoints) {
  ...
  return Object.keys(endpoints).reduce(function (acc, key) {
    var _endpoints$key = endpoints[key],
        ...
        _endpoints$key$noEnco = _endpoints$key.noEncode,
        noEncode = _endpoints$key$noEnco === undefined ? [] : _endpoints$key$noEnco;
    ...
    acc[key] = function () {
      ...
      var req = _request2.default.apply(undefined, _toConsumableArray(beforeRequest(baseUrl, (0, _applyUrlWithPlaceholders2.default)(path, placeholders, noEncode), configureOptions(augmentedOptions))));
